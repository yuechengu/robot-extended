...
def runRF():    
    ...
    # 先去掉 extend 自己的参数 和 脚本名参数
    
    print(sys.argv)
    
    argsForRf = [arg for arg in  sys.argv if arg not in EXTENDED_ARGS][1:]
    ...
    if GEN_DIR:
        cmd = f'{python_executable} -m robot.run -d ./reports/ {argStr}  {CASE_GEN_DIR}'
    else:
        cmd = f'{python_executable} -m robot.run -d ./reports/ {argStr}  {CASE_DIR}'
